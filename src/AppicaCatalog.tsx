...
function Preview({ slug, language }: { slug: string; language: AppicaCatalogLanguage }) {
  ...
  const previews: Record<string, ReactNode> = {
    ...
    collapsible: <Collapsible defaultOpen><CollapsibleTrigger>{text("Project details", "项目详情")}</CollapsibleTrigger><CollapsibleContent><p className="appica-compact-copy">{text("Three environments · 12 members", "3 个环境 · 12 位成员")}</p></CollapsibleContent></Collapsible>,
    ...
    menubar: <Menubar><MenubarMenu><MenubarTrigger>{text("File", "文件")}</MenubarTrigger><MenubarContent><MenubarItem>{text("New", "新建")}</MenubarItem><MenubarItem>{text("Open", "打开")}</MenubarItem></MenubarContent></MenubarMenu><MenubarMenu><MenubarTrigger>{text("Edit", "编辑")}</MenubarTrigger><MenubarContent><MenubarItem>{text("Undo", "撤销")}</MenubarItem><MenubarItem>{text("Redo", "重做")}</MenubarItem></MenubarContent></MenubarMenu></Menubar>,
    meter: <Meter value={72}><MeterLabel>{text("Storage", "存储空间")}</MeterLabel><MeterValue /><MeterProgress /></Meter>,
    navigation: <Navigation aria-label={text("Preview navigation", "预览导航")} activeLink="components"><NavigationList><NavigationItem><NavigationLink href="#home" value="home">{text("Home", "首页")}</NavigationLink></NavigationItem><NavigationItem><NavigationLink href="#components" value="components">{text("Components", "组件")}</NavigationLink></NavigationItem></NavigationList></Navigation>,
    "navigation-menu": <NavigationMenu><NavigationMenuList><NavigationMenuItem><NavigationMenuTrigger>{text("Products", "产品")} <NavigationMenuIcon /></NavigationMenuTrigger><NavigationMenuContent><NavigationMenuLink href="#agent">Agent UI</NavigationMenuLink><NavigationMenuLink href="#design">{text("Design system", "设计系统")}</NavigationMenuLink></NavigationMenuContent></NavigationMenuItem></NavigationMenuList></NavigationMenu>,
    ...
    tabs: <Tabs defaultValue="preview"><TabsList><TabsTrigger value="preview">{text("Preview", "预览")}</TabsTrigger><TabsTrigger value="code">{text("Code", "代码")}</TabsTrigger></TabsList><TabsContent value="preview"><p className="appica-compact-copy">{text("Live component result", "组件实时效果")}</p></TabsContent><TabsContent value="code"><code>&lt;Component /&gt;</code></TabsContent></Tabs>,
    ...
    toggle: <div className="appica-preview-row"><Toggle aria-label={text("Bold", "粗体")}><strong>B</strong></Toggle><Toggle aria-label={text("Italic", "斜体")}><em>I</em></Toggle></div>,
    "toggle-group": <ToggleGroup className="appica-centered-control" aria-label={text("Alignment", "对齐方式")} defaultValue={["center"]}><Toggle value="left" aria-label={text("Left", "左对齐")}>L</Toggle><Toggle value="center" aria-label={text("Center", "居中")}>C</Toggle><Toggle value="right" aria-label={text("Right", "右对齐")}>R</Toggle></ToggleGroup>,
    toolbar: <Toolbar aria-label={text("Formatting", "格式工具")}><ToolbarGroup aria-label={text("Text style", "文字样式")}><ToolbarButton render={<Button variant="ghost">{text("Bold", "粗体")}</Button>} /><ToolbarButton render={<Button variant="ghost">{text("Italic", "斜体")}</Button>} /></ToolbarGroup><ToolbarSeparator /><ToolbarLink href="#help">{text("Help", "帮助")}</ToolbarLink></Toolbar>,
